...
In the reference app, single-line fields are underlined and multi-line fields are boxed. Underlined fields carry a small radius so their focus ring has the same soft corners a textarea's does, and no field sets its own focus colour — the one rule above owns that. Production's `Input` is boxed rather than underlined, so every text control renders differently in the two apps; which of the two is the target has not been decided.
