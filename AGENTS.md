...
- An integration test drives the application through an entry point and nothing else. Entry points are today's API routes, and later any other way in, such as a websocket. Build the real container through `PlatformIntegrationTestContext.createContainer` against ephemeral infrastructure from testcontainers, call the entry point, and assert its response together with the side effects that reached the database or a provider.
- Integration tests mock nothing. The database and every third-party service run as containers — a third party as a mock-endpoint server such as MockServer, honoring its real contract — and the application reaches them through its real adapter, pointed there by configuration. Assert what the service actually received. Substitute only in-process inputs no container can stand in for: randomness and wall-clock time.
