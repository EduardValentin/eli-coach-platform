...
| `Button` | Every text action control, as a button or through `buttonVariants` on a link | `variant`: `primary`, `secondary`, `inverted`, `outline`, `outline-brand`, `glass`; `size` by height: `xs` (32px), `md` (48px), `lg` (56px), `xl` (60px); `width`: `content`, `full`; `weight`: `regular`, `medium`, `semibold`; `textSize`: `sm`, `base`, `lg`; `lettering`: `plain`, `nav`, `caps`; `elevation`: `flat`, `raised`; `press`: `none`, `scale` |
...
Every button shares one base: the 14px `control` corner, a 2-unit gap, no wrapping, 50% opacity when disabled, and the one focus rule below. A size sets only the height and one horizontal padding (`px-4` for `xs`, `px-8` otherwise); a `full` button fills its row and drops that padding. `raised` is the only elevation: the `action` shadow lifting to `action-hover` on hover. `glass` is the one translucent tone for an action over the hero; the header's portal link uses it while the header is transparent and `primary` once it turns solid. The reference app's `ThemeButton` mirrors production's `Button` variant for variant, and its shadcn `Button` keeps the same corner and no focus border.
...
Layout tokens include `container-reading`, `container-content`, and `container-stage`; `size-control-{xs,sm,md,lg,xl}` and `size-avatar-{sm,md,lg}`. Radii are named by role, never by size: `checkbox` (4px) the check box, `tile` (6px) small tiles, tags, the logo mark and chat-bubble tails, `field` (8px) text fields and the thumbnails and rows inside a card, `compact` (10px) the pricing card's folder-tab badge and small brand marks, `control` (14px) every button, time slot and calendar control, `card` (16px) every card and card-like surface, `panel` (24px) large framed panels, and `phone-frame` (40px) the device chrome. The reference app's shadcn size steps resolve to these roles (`sm` tile, `md` field, `lg` compact, `xl` control, `2xl` card, `3xl` panel); production uses the role names only. Fully rounded shapes use the framework's `rounded-full`; and `shadow-{soft,card,raised,floating,action,action-hover,phone-frame}`, where `card` is the low elevation of cards and chosen options and `action`/`action-hover` the resting and hover lift of a raised call to action. Public-site compositions use narrowly scoped `public-*` radius, size, and shadow tokens rather than adding raw repeated values.
...
- `tracking-label`, `tracking-nav`, and `tracking-section-eyebrow` cover the letter-spacing roles; the framework's `tracking-*` steps keep their own values.
