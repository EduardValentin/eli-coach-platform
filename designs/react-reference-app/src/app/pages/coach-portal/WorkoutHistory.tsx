...
function SubscriptionScopeLabel({
  subscription,
}: {
...
export function WorkoutHistory() {
  ...
  const { getClientWorkoutHistory, exercises, planInstances, subscriptions } =
    useTraining();
  ...
  const planById = useMemo(
    () => new Map(planInstances.map((p) => [p.id, p])),
    [planInstances],
  );
  ...
  const clientPlans = useMemo(
    () => planInstances.filter((p) => p.clientId === dataClientId),
    [planInstances, dataClientId],
  );
  ...
  const plansInScope = useMemo(
    () =>
      (subscriptionScope === 'all'
        ? clientPlans
        : clientPlans.filter((p) => p.subscriptionId === subscriptionScope)
      )
        .slice()
  ...
  const selectedSub =
    subscriptionScope === 'all' ? undefined : subById.get(subscriptionScope);
  ...
  const [volumeRangeKg, setVolumeRangeKg] = useState<[number, number]>([
    volMinKg,
    volMaxKg,
  ]);
  ...
  const durationTouched =
    durationRange[0] > durMin || durationRange[1] < durMax;
  const volumeTouched =
    volumeRangeKg[0] > volMinKg || volumeRangeKg[1] < volMaxKg;
  ...
  const filtered = useMemo(
    () =>
      history.filter((w) => {
        if (
          subscriptionScope !== 'all' &&
          planById.get(w.planInstanceId)?.subscriptionId !== subscriptionScope
        )
          return false;
        ...
        if (selectedMuscles.length) {
          ...
          w.exercises.forEach((el) =>
            exercises
              ...
              ?.primaryMuscles.forEach((m) => ms.add(m)),
          );
  ...
  const topExercises = useMemo(() => {
    ...
    filtered.forEach((w) =>
      w.exercises.forEach((el) => {
        freq[el.exerciseId] = (freq[el.exerciseId] || 0) + 1;
      }),
    );
  ...
  const groups = useMemo<SessionGroup[]>(() => {
    ...
    filtered.forEach((w) => {
      ...
      const sub = plan?.subscriptionId
        ? subById.get(plan.subscriptionId)
        : undefined;
  ...
  const volStep = Math.max(
    1,
    Math.round((volDisplayMax - volDisplayMin) / 100),
  );
          ...
          <div className="flex flex-col lg:flex-row lg:items-center gap-3 mb-3">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 lg:flex lg:items-center flex-1 min-w-0">
              ...
              >
                ...
                >
                  {selectedSub ? (
                    <SubscriptionScopeLabel subscription={selectedSub} />
                  ) : (
                    'All subscriptions'
                  )}
                </SelectTrigger>
              ...
              <Select value={planScope} onValueChange={setPlanScope}>
                <SelectTrigger
                  className="lg:w-64"
                  aria-label="Filter by training plan"
                >
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
              ...
              <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                ...
                <FilterField icon={<Dumbbell size={14} />} label="Volume">
                  <RangeControl
                    ...
                    onValueChange={(v) =>
                      setVolumeRangeKg([
                        fromDisplayWeight(v[0], weightUnit),
                        fromDisplayWeight(v[1], weightUnit),
                      ])
                    }
                  ...
                  <div className="space-y-3">
                    {g.logs.map((log) => (
                      <WorkoutSessionCard
                        key={log.id}
                        log={log}
                        to={`/coach/clients/${clientId}/workout/${log.id}`}
                      />
                    ))}
                  </div>
                </section>
