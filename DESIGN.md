...
- **My Method graph tokens:** Use the `public-my-method-*` token set for the decorative progress graph so graph geometry, graph-card elevation and border, heading rhythm, and graph micro-typography are named by role rather than copied as one-off component values.
