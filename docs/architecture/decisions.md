...
| `packages/domain/src/shared` is interface-only (R5): `Clock` (implemented in `apps/platform/src/server/container.server.ts` as the system clock; consumed by `WaitlistService`, `StoreAcquisitionService` and `DownloadGrantService` through `waitlist-composition.server.ts` and `store-composition.server.ts`), `Logger` (implemented by `createConsoleLogger` in `apps/platform/src/server/logger.server.ts`; consumed by `WaitlistService` and `StoreAcquisitionService`), `BotVerifier` (implemented by `StaticTokenBotVerifier` and `TurnstileBotVerifier` in `packages/infrastructure/src/bot-detection`, selected by `createBotVerifier`; consumed by the waitlist and store acquisitions controllers), `ProductEmail` (implemented by `ResendProductEmailSender` and `InMemoryProductEmailSender` in `packages/infrastructure/src/email`, selected by `createProductEmail`; consumed by the waitlist and store email services), `ManagementAuthenticator` (implemented by `BearerSecretManagementAuthenticator` in `packages/infrastructure/src/management-auth`; consumed by the store management controller) | a slice that needs a cross-cutting concern imports a stable interface rather than another slice's implementation | none: `/shared` has no executable rules, so nothing but the port shapes cross the boundary | Eduard | 2026-09-17 |
| The `stability` rule (R31) excludes same-directory-or-descendant edges (`pathNot` keyed on the `from` module's containing directory) | Every package or subpath barrel (`index.ts`, `index.server.ts`) and hub aggregator (`packages/config/src/runtime.ts`, `packages/infrastructure/src/bot-detection/bot-detection-widget.tsx`) is, by construction, more depended-on and therefore more stable than the narrower implementation files it re-exports or composes in the same directory; the cold run reported 45 such edges (every subpath barrel in `packages/ui`, `packages/infrastructure`, `packages/domain`, `packages/db`, `packages/content`, `packages/config` against its own implementation modules) and none from a different direction | genuine cross-package and cross-slice instability (a domain slice depending on `/shared`, infrastructure depending on config or domain) remains enforced; only the unavoidable barrel-to-implementation direction is exempted | Eduard | 2026-09-17 |
