...
- The application under test runs in its own process, so nothing in a test can substitute an in-process input for it. Arrange time the way the application observes it: age the rows whose timestamps it measures against, and leave the exact edges of a window — the millisecond a cooldown expires, the instant a bucket starts — to unit tests over the domain service that computes them. Never reach into the running instance to move its clock.
