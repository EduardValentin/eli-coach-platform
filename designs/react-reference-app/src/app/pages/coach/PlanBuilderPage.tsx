  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap gap-1 mt-auto">
        {ex.tags?.map(t => (
          <span key={t} className="text-[9px] bg-[#C81D6B]/10 text-[#C81D6B] px-1.5 py-0.5 rounded">{t}</span>
        ))}
        {ex.primaryMuscles.map(m => (
          <span key={m} className="text-[9px] bg-neutral-100 text-neutral-600 px-1.5 py-0.5 rounded">{m}</span>
        ))}
      </div>
    </div>
  );
...
function PlanGroupCard({
  ...
  return (
    ...
    >
      {group.isSuperset && (
        <div className="bg-[#00796B] text-white px-4 py-2 rounded-t-xl flex justify-between items-center text-xs font-bold uppercase tracking-wider cursor-grab active:cursor-grabbing" ref={drag as any}>
...
export function PlanBuilderPage() {
  ...
  // Week Action Dropdown State
  const [openWeekAction, setOpenWeekAction] = useState<number | null>(null);

  useEffect(() => {
    // Initialize 4 weeks
    const initialWeeks: PlanWeek[] = Array.from({ length: 4 }).map((_, i) => ({
      id: `w-${Date.now()}-${i}`,
      order: i + 1,
      isDeload: i === 3,
  ...
  const handleSwapWeek = (idxA: number, idxB: number) => {
    ...
    setWeeks(prev => {
      ...
      w[idxA].days = w[idxB].days;
      w[idxB].days = tempDays;
      return w;
    });
  ...
  const handleDropOnGroup = (dragItem: any, targetGroupId: string) => {
    ...
    if (dragItem.type === 'LIBRARY_EXERCISE') {
        draggedItems = [{ id: `pe-${Date.now()}`, exerciseId: dragItem.exercise.id, sets: 3, reps: '10', rir: 2 }];
    } else if (dragItem.type === 'PLAN_EXERCISE') {
        const oldIndex = newGroups.findIndex(g => g.id === dragItem.id);
        if (oldIndex === -1) return;
        const [movedGroup] = newGroups.splice(oldIndex, 1);
  ...
  const handleSavePlan = (isDraft: boolean = false) => {
    if (!name.trim()) {
      toast.error('Please name the plan');
      return;
    }
    addPlan({
      id: `p-${Date.now()}`,
    ...
    toast.success(isDraft ? 'Plan saved as draft!' : 'Plan saved successfully!');
        ...
        <div className="h-16 px-6 border-b border-neutral-200 bg-white flex items-center justify-between shrink-0 sticky top-0 z-30">
          <div className="flex items-center gap-4">
            ...
            <input
              ...
              className="text-xl font-serif font-bold text-[#121212] focus:outline-none placeholder:text-neutral-300 bg-transparent w-full max-w-md"
            />
          </div>
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          <div className="w-72 bg-white border-r border-neutral-200 flex flex-col shrink-0">
                          ...
                          {openWeekAction === wIdx && (
                            ...
                            >
                              ...
                              <div className="border-t border-neutral-100 mt-1">
                                <button onClick={() => handleRemoveWeek(wIdx)} className="w-full text-left px-4 py-2 text-sm hover:bg-red-50 text-red-600 flex items-center gap-2">
                        ...
                        return (
                          ...
                          >
                            ...
                            <span className={`text-[10px] px-1.5 py-0.5 rounded uppercase tracking-wider font-semibold ${
                              ...
                              day.type === 'Hypertrophy' ? 'bg-[#00796B]/10 text-[#00796B]' :
                              day.type === 'Recovery' ? 'bg-green-100 text-green-700' :
            ...
            <div className="p-4 border-t border-neutral-200 bg-neutral-50 shrink-0">
              <button onClick={handleAddWeek} className="w-full py-2.5 flex items-center justify-center gap-2 bg-white hover:bg-neutral-100 text-[#121212] font-semibold text-sm rounded-xl transition-colors border border-neutral-200 shadow-sm">
                ...
                {activeDay.type === 'Rest' ? (
                  ...
                  <div className="space-y-2 pb-32">
                    {selectedForSuperset.length > 0 && (
                      ...
                      >
                        <span className="text-sm font-semibold text-[#C81D6B]">{selectedForSuperset.length} exercises selected</span>
                        <div className="flex gap-3">
                          <button onClick={() => setSelectedForSuperset([])} className="px-4 py-2 text-sm font-semibold text-neutral-600 hover:bg-neutral-100 rounded-lg transition-colors">Cancel</button>
                          <button onClick={handleGroupSuperset} className="px-4 py-2 text-sm font-semibold bg-[#C81D6B] text-white rounded-lg shadow-sm hover:bg-[#a31556] transition-colors">Create Superset</button>
                        </div>
                      </motion.div>
                    )}
          ...
          <div className="w-80 bg-white border-l border-neutral-200 flex flex-col shrink-0 z-10 shadow-[-4px_0_15px_-3px_rgba(0,0,0,0.05)]">
