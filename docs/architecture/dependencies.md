...
Header: date 2026-09-18, commit bcf6555e plus the GEN-191 booking-card rebuild (`bcf6555e..dde3b960`), scope apps/platform/src, apps/platform/db, packages/{config,content,db,domain,infrastructure,test-support,ui}/src plus the enforcement layer (tools/dependency-cruiser.config.cjs, tools/dependency-cruiser.tsconfig.json, tools/boundaries.test.mjs, tools/boundary-fixtures/, tools/domain-layout.mjs, tools/domain-layout.test.mjs, tools/domain-layout-fixtures/, knip.json, eslint.config.mjs, workspace package.json export maps, tsconfigs, vite/react-router/vitest configs), mode record update for GEN-191 over `843d8261..bcf6555e` and the booking-card rebuild on top of it — the slices recorded at 422c7797, corrected after the two review fix rounds (`422c7797..186b77c4`), the remediation round (`186b77c4..6fc3157f`) and the booking-card rebuild — on top of change review run 8 at e8690f45.

## Component graph

Generated from a cold cruise of the booking-card rebuild's working tree on bcf6555e (363 modules, 946 dependencies, 0 violations, 0 circular, 0 unresolved, over 340 in-scope production modules; the per-module edge table is below). Count = distinct importing modules. Component IDs refer to `components.md`. Every count C17 does not appear in is unchanged from run 8 at e8690f45: `features/assessment-calls` is the only component the GEN-191 window added, and nothing else gained or lost a cross-component importer. The review fix round moved two C17 counts: C17 → C1 went 12 → 13 (`ui/public/book/call-facts.tsx` reads the duration off `ASSESSMENT_CALL_RULES`) and C17 → C5 went 5 → 6 (the join page took `Link` from `./primitives`, while `booking-details-form.tsx` dropped its `./lib` import). The remediation round took C17 → C5 back to 5: the join page no longer renders a page of its own, so it imports nothing from C5. The booking-card rebuild moved both again: C17 → C1 went 13 → 12 (`call-facts.tsx` was deleted and `slot-grouping.ts` lost its `ASSESSMENT_CALL_RULES` import, while `call-overview.tsx` took the duration) and C17 → C5 went 5 → 6 (`slot-picker.tsx` takes `cn` from `./lib`, which the page, the details form and the confirmation import again too).
