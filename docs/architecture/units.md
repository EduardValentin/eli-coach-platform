...
| U519 | server/logger.server.ts:createConsoleLogger (+private ConsoleLogger) | C14 | adapter | adapters | container-only | operator/platform | altered in a79f507d; structurally implements U1193 and U1194; changed (the main merge; adds `waitlistModeReadFailed`) |
...
| U1194 | domain/waitlist/waitlist-incidents.ts:WaitlistIncidents (`confirmationDeliveryFailed()`, `waitlistModeReadFailed()`) | C1 | port | use-cases | published (`./waitlist`) | operator/platform | added in a79f507d; changed (the main merge; adds `waitlistModeReadFailed` for U963) |
...
| U1203 | apps/platform/e2e/support/waitlist-mode.ts:disableWaitlistMode, restoreWaitlistMode | tests | test adapter | tests | E2E-private | operator/platform | added (621cdc7e); changed (f0eb1bf4; setup records the prior `WAITLIST_MODE` value in the runner environment and teardown writes that value back); changed (f0eb1bf4; the boolean-parameter update became separately named switch-on and switch-off updates) |
