    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Set the category, reference macros per 100 g, swap group, and tags
            for this food.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="grid gap-4">
          ...
          <div className="grid gap-2">
            ...
            <Select
              ...
              onValueChange={(v) => set('category', v as FoodCategory)}
            >
              ...
              {FOOD_ICONS.map(({ key, label, Icon }) => (
                <li key={key}>
                  <button
                    ...
                    className={`flex size-9 items-center justify-center rounded-compact border transition-colors ${draft.icon === key ? 'border-primary bg-primary-soft text-primary' : 'border-border text-text-secondary hover:bg-surface-muted'}`}
                  >
          ...
          <fieldset className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <legend className="text-sm font-medium mb-1 col-span-full">
              Macros (per 100 g)
            </legend>
            {(
              [
          ...
          <div className="grid gap-2 max-w-[12rem]">
            ...
            <Input
              ...
              min="0"
              inputMode="numeric"
              value={draft.defaultPortionGrams}
              onChange={(e) => set('defaultPortionGrams', e.target.value)}
            />
          </div>
          ...
          <div className="grid gap-2">
            ...
            <Select
              value={draft.equivalenceGroupId || 'none'}
              onValueChange={(v) =>
                set('equivalenceGroupId', v === 'none' ? '' : v)
              }
            >
              <SelectTrigger id="food-group">
                  ...
                  <ul className="flex flex-wrap gap-1.5">
                    {tags
                      ...
                      .map((t) => (
                        <li key={t.id}>
                          <ToggleChip
                            pressed={draft.tagIds.includes(t.id)}
