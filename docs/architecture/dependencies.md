...
| C17 assessment-calls | C1 domain | 12 | `/assessment-call`, `/coach-availability`, `/shared`; the controller, the wire contract, the composition, the three `data/` adapters, the notification factory and adapter, the calendar-invite and the two content builders, and in the browser half `call-overview.tsx` alone, reading `ASSESSMENT_CALL_RULES` |
...
| E1197 | apps/platform/src/features/assessment-calls/ui/public/book/call-overview.tsx | packages/domain/src/coach-availability/index.ts | import | yes | yes | lateral | present |
