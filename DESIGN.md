...
- Production tokens: `packages/ui/src/styles.css`. Production components and variants: `packages/ui` has no root barrel; it publishes ten concern subpaths instead (`appointments`, `calendar`, `filters`, `layout`, `lib`, `motion`, `overlays`, `primitives`, `tabs`, `toast`), each imported on its own.
