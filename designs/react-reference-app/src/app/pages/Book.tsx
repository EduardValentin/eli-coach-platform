...
// Mock generation of time slots based on a date seed to keep it consistent-ish but varied
const generateTimeSlots = (date: Date | undefined): TimeSlot[] => {
  if (!date) return [];
  if (isWeekend(date)) return []; // No weekends available
  
  const seed = date.getDate();
  const allSlots = [
    '09:00 AM', '09:30 AM', '10:00 AM', '11:00 AM', 
    '01:00 PM', '02:30 PM', '03:00 PM', '04:00 PM', '04:30 PM'
  ];

  return allSlots.map((time, i) => {
    // Arbitrary logic to simulate some slots being booked
    const isBooked = (seed + i) % 4 === 0 || (seed % 2 !== 0 && i === 2);
                ...
                <div className="flex flex-col lg:flex-row gap-8 lg:gap-12 flex-1">
                  ...
                  <div className="flex-1">
                    <Calendar
                      ...
                      disabled={(date) => date < today || date > maxDate || isWeekend(date)}
                  ...
                  {selectedDate && (
                    ...
                    >
                      ...
                      <div className="flex flex-col gap-2 max-h-[350px] overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-neutral-200 scrollbar-track-transparent">
                        {availableSlots.length > 0 ? (
                          availableSlots.map((slot, i) => (
                            <div key={i} className="flex gap-2">
                              ...
                              {/* Quick Next Button reveals only for selected slot */}
                          ...
                          <p className="text-sm text-neutral-500 italic">No available slots for this date.</p>
                        )}
                      </div>
                    </motion.div>
                  )}
