...
- **Food-category colors (Nutrition module):** `nutrition-protein` (#b91c1c), `nutrition-carb` (#c2410c), `nutrition-fat` (#a16207), `nutrition-legume` (#15803d), `nutrition-extra` (#7e22ce), `nutrition-seasoning` (#0f766e), each with a `-soft` 10% tint. Identity colors for Food Library categories — use for category swatches, soft badge fills, and left-border accents only. Category labels stay `text-foreground` for AAA contrast; never render a category name as small text on its own soft tint. Consume in raw `var()` via the short name `var(--nutrition-*)` (the `--color-*` alias is build-time only). Defined once, light/dark-safe.
