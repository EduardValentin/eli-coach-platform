...
export function CycleSyncing() {
  ...
  // Start at Day 25 (approaching menstrual phase)
  const START_DAY = 25;
  ...
  // Derive current day strictly from rotation so it perfectly matches visual state
  const currentDay = useMemo(() => {
    const normalizedDegrees = ((-targetRotation % 360) + 360) % 360;
    let day = Math.round(normalizedDegrees / DEGREES_PER_DAY) + 1;
    if (day > 28) day = 1;
    return day;
  }, [targetRotation]);
  ...
  useEffect(() => {
    ...
    const SCROLL_THRESHOLD = 30; // Sensitivity for triggering a day change
    ...
    const handleWheelEvent = (e: WheelEvent) => {
      ...
      if (Math.abs(scrollAccumulator) >= SCROLL_THRESHOLD) {
        ...
        // Snap precisely to the next/prev day
        setTargetRotation(prev => prev - (direction * DEGREES_PER_DAY));
      }
    };
  ...
  // Calculate colors/patterns matching Apple Health style
  const getPillStyle = (day: number) => {
    // Menstrual Phase (Solid red, intensity decreasing)
    if (day >= 1 && day <= 5) {
      return { bg: '#FF4D6D', isStriped: false, opacity: 1 - (day - 1) * 0.1 };
    }
    // Approaching Menstrual (Striped red, intensity increasing)
    if (day >= 23 && day <= 28) {
      return { bg: '#FF4D6D', isStriped: true, opacity: 0.3 + (day - 23) * 0.12 };
    }
    // Just after Menstrual (Striped red fading out)
    if (day >= 6 && day <= 8) {
      return { bg: '#FF4D6D', isStriped: true, opacity: 0.5 - (day - 6) * 0.15 };
    }
    // Other phases (Subtle phase color)
  ...
  return (
    <section className="py-24 bg-[#FAFAFA] overflow-hidden min-h-[800px] flex items-center">
      <div className="max-w-[1440px] w-full mx-auto px-6 lg:px-24 flex flex-col lg:flex-row items-center gap-16 lg:gap-24">
        ...
        <div className="flex-1 relative w-full max-w-[500px] aspect-square flex items-center justify-center mt-10 lg:mt-0">
          ...
          {/* Top Indicator - Sleek exact pointer */}
          <div className="absolute top-[-16px] left-1/2 -translate-x-1/2 z-30 flex flex-col items-center drop-shadow-md">
            <svg width="20" height="14" viewBox="0 0 20 14" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M10 14L0 0H20L10 14Z" fill="#121212" />
            </svg>
          </div>

          {/* Wheel Container - Perfect Circle */}
          <div 
            ref={containerRef}
            className="relative w-full h-full rounded-full touch-none select-none cursor-ns-resize"
          >
            ...
            >
              {dots.map(day => {
                ...
                return (
                  ...
                  >
                    {/* The Pill positioned exactly on the radius edge */}
                    <div className="absolute top-1 left-1/2 -translate-x-1/2">
                      ...
                      >
                         {/* Apple Health Style Inner Circle */}
                         <div 
