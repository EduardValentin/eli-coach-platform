...
describe("BundleSelector", () => {
  it("renders the public bundle cards and shared benefits once", () => {
    ...
    expect(
      screen.getAllByText("Personalized workout and nutrition program"),
    ).toHaveLength(1);
    expect(screen.getAllByText("Access to the private community")).toHaveLength(
      1,
    );
  });
  ...
  it("shows all-bundle waitlist pricing in waitlist mode", () => {
    ...
    expect(
      screen.getByLabelText("Original 3 months monthly price €149"),
    ).toBeInTheDocument();
    expect(
      screen.getByLabelText("3 Months monthly price €125"),
    ).toBeInTheDocument();
    expect(
    ...
    expect(
      screen.getByLabelText("Original 6 months monthly price €139"),
    ).toBeInTheDocument();
    expect(
      screen.getByLabelText("6 Months monthly price €119"),
    ).toBeInTheDocument();
    expect(
