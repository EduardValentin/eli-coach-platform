...
- **My Method graph tokens:** Use `ratio-public-my-method-graph` and the `position-public-my-method-*` label-placement tokens for the decorative progress graph so the prototype geometry is named by role rather than copied as one-off component values.
