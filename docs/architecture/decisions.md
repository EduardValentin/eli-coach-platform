...
| Email templates render React on the server (`email-primitives.server.tsx`) and in each feature's `email/` content builders | React as an HTML templating engine, never hydrated | framework in an adapter package; accepted by design | implicit | 2026-08-09 |
...
| The `AllowAllManagementAuthenticator` double the spec §3.5 places in `packages/test-support` was deleted for want of a consumer | no code exists only to be available | tests construct their own fake; re-add it when a test needs it | Eduard | 2026-09-17 |
...
| Target (path and symbol, or edge) | Rule | Reason accepted | Accepted by | Date |
|---|---|---|---|---|
| `apps/platform/src/**/**-controller.server.ts` → `packages/domain/src/**/*-use-case.ts` | R6 | Single-implementation use cases in a pre-launch MVP; TypeScript treats the class types structurally and tests substitute doubles | Eduard | 2026-09-16 |
| `packages/infrastructure/src/email/email-primitives.server.tsx` and feature `email/*-email.server.tsx` | R21 | React is the server-side HTML templating mechanism; these modules are adapters, not hydrated views | Eduard | 2026-09-16 |
| controllers and adapter catch sites under `apps/platform/src/features` and `packages/infrastructure` | R39 | Controller shape and infrastructure-failure mapping remain review-owned; only named expected conditions such as filesystem ENOENT may be classified | Eduard | 2026-09-16 |
...
- Single-coach platform for its lifetime. The model stays single-tenant; no owning-coach references are introduced. This removes the former single-coach versus multi-coach deferred question.
- Fixed rather than accepted: explicit `BOT_DETECTION_PROVIDER`; dependency-cruiser and knip as typecheck/build gates; domain-owned ports with domain names; unused UI exports removed and the UI barrel split by concern.
...
- Route thinness is enforced by the `route-thinness` dependency rule instead of review.
...
- The allow-all management authenticator double is not kept in `@eli-coach-platform/test-support` until a test needs it.
