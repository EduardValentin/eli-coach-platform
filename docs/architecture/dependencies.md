...
| C17 assessment-calls | C1 domain | 12 | `/assessment-call`, `/coach-availability` (`StaticCoachAvailability` alone), `/shared`; the controller, the wire contract, the composition, the three `data/` adapters, the notification factory and adapter, the calendar-invite and the two content builders, and in the browser half `call-overview.tsx` alone, reading `ASSESSMENT_CALL_RULES` |
...
| C17 assessment-calls | C6 infrastructure | 10 | bot-detection (browser and server), email/server, http/server, coach-calendar/server (the repository's reservation writers and the composition's `PostgresCoachCalendar`) |
...
| B241 | ProductEmail (U960) | C1 `/shared` | U1023 ResendProductEmail, U1038 InMemoryProductEmail, behind `createProductEmail` | U129, U307, U1211 adapters | ProductEmailCommand / ProductEmailResult. The command carries an optional per-send `replyTo`; an implementer that sends honours it and otherwise falls back to its configured reply-to address. `ProductEmail.provider` is boundary data the use case records in its delivery audit, not a detail leak | implementer | exports |
...
| B260 | DatabaseClient (U1151, Drizzle NodePgDatabase) | C2 adapters | drizzle() | U503; every repository in C7, C8, C9, C17, C6 | Drizzle ORM instance (detail type). `DatabaseTransaction`, the handle a `transaction` callback receives, is published beside it and named by U1279, U1206 and U125 | C2 | exports |
| B261 | appSchema (U1154, the `app` Postgres namespace) | C2 frameworks | tables attached by U126, U304, U407, U1028, U1280 | same five | drizzle PgSchema builder | consumers | convention; drizzle.config.ts globs discover the tables |
...
An edge from A to B means A's source names B. Direction `inward` points toward policy (ring order: entities, use-cases, adapters, frameworks, composition). Generated from every `import`, `export … from`, dynamic `import()` and CSS `@import` in the 351 in-scope production modules of the GEN-191 fix round at 59c019ec, one row per module pair (a module that imports the same target twice, once type-only, still has one row); kind is `import` for all rows (the `implements` and `constructs` relationships are recorded in the Boundaries and Entry points sections above). Crosses-ring compares the majority ring of the two modules from `units.md`; an edge into a `packages/domain` folder entry is recorded `lateral`, because a subpath barrel is a publication surface rather than a ring of its own. Externals are tagged framework, vendor or runtime. The cruise resolves every specifier (zero `couldNotResolve`), but `options.exclude.path` in `tools/dependency-cruiser.config.cjs` carries `"\\.d\\.ts$"`, and the cruiser applies it to resolved targets as well as to sources: an external whose resolution lands on a `.d.ts` file is dropped with its edges. That is why `react-router`, `@react-router/dev`, `@clerk/react-router`, `motion`, `react-day-picker`, `drizzle-orm`, `react-hook-form`, `@hookform/resolvers` and `tailwind-merge` are named in the per-component external list above but have no `external:` row here; packages that resolve to `.js`, `.mjs`, `.d.mts` or `.d.cts` (`zod`, `resend`, `radix-ui`, `clsx`, `@date-fns/tz` among them) keep theirs. The per-component list is read from each module's source, not from this table. Component membership is by path (see `components.md`). Edge IDs are stable across runs: 491 of these rows keep the ID run 6 gave them, E738-E1013 came in with run 8, E1014-E1165 came in with GEN-191's slices, and E1166-E1180 came in with its review fix round, which also deleted seven rows for edges the code no longer has and one duplicate `booking-details-form.tsx → react` row. E1181-E1183 came in with the remediation round (`book-page.tsx → step-heading-focus.ts`, `step-heading-focus.ts → react`, `booking-confirmation.tsx → react`), which deleted E1175 (`join-page.tsx → packages/ui` primitives) and E1177 (`textarea.tsx → class-variance-authority`, an edge the second fix round had already removed from the code). E1184-E1205 came in with the booking-card rebuild, which deleted twelve rows for edges the code no longer has: E1091, E1093 and E1168 (the page's imports of `slot-calendar.tsx`, `slot-list.tsx` and `call-facts.tsx`), E1169, E1170 and E1174 (the confirmation, the details form and the slot list into `contracts/call-moment.ts`), E1171 and E1172 (`call-facts.tsx`), E1118 (`slot-grouping.ts` into C1), E1162 and E1164 (`calendar.tsx` into `react` and `./primitives`), and E1180 (`textarea.tsx → input.tsx`). E1206-E1213 came in with the second parity round (`7a61673f..d3513898`): the three dead-end pages into `./layout` (E1206, E1207, E1209), the waitlist email form into `./motion` (E1208), `about.tsx` into `./primitives` (E1210), and the new `layout/dead-end-page.tsx` into `react` and `./primitives` and from the `./layout` entry (E1211-E1213). It deleted nine rows for edges the code no longer has: E45, E49 and E298 (the same three pages into `./primitives`), E286 (the email form into `./primitives`), E413 (`about.tsx` into `./motion`), E445 and E447 (`hero.tsx` into `./lib` and `./primitives`), E861 (`platform-content.ts → react`, whose icon type is now lucide's) and E1008 (`input.tsx → class-variance-authority`, gone with the input's variants). E1261-E1267 came in with the GEN-191 fix round (`47edb229..59c019ec`): `auth-nav-actions.tsx` and `catalog-view.tsx` into `./primitives` (E1261, E1263), `book-page.tsx` into its new `book-page.css` (E1262), `logo.tsx`, `public-layout.tsx` and `public-navigation.tsx` into the new `header-appearance.ts` (E1264-E1266), and the C2 entry into the new `database-error.ts` (E1267). It deleted four rows for edges the code no longer has: E1218, E1191, E1228 and E1234 (`access-denied-page.tsx`, `booking-confirmation.tsx`, `root-error-page.tsx` and `pricing.tsx` into `./lib`). E1246, E1248, E1249 and E1252 keep their IDs with their target renamed from `busy-interval.ts` to `time-interval.ts`. A row whose source module was only edited keeps its old ID. The generating command is `npx depcruise --config tools/dependency-cruiser.config.cjs --output-type json apps/platform/src packages/{config,content,db,domain,infrastructure,test-support,ui}/src`.
...
| E1261 | apps/platform/src/features/accounts/ui/public/auth-nav-actions.tsx | packages/ui/src/primitives/index.ts | import | yes | no | lateral | present |
...
| E1206 | apps/platform/src/features/accounts/ui/public/sign-in-failed-page.tsx | packages/ui/src/layout/index.ts | import | yes | no | lateral | present |
| E1214 | apps/platform/src/features/accounts/ui/public/sign-in-failed-page.tsx | packages/ui/src/primitives/index.ts | import | yes | no | lateral | present |
...
| E1207 | apps/platform/src/features/accounts/ui/shared/access-denied-page.tsx | packages/ui/src/layout/index.ts | import | yes | no | lateral | present |
| E1219 | apps/platform/src/features/accounts/ui/shared/access-denied-page.tsx | packages/ui/src/primitives/index.ts | import | yes | no | lateral | present |
| E1236 | apps/platform/src/features/assessment-calls/contracts/assessment-calls.ts | packages/domain/src/assessment-call/index.ts | import | yes | no | lateral | present |
| E1237 | apps/platform/src/features/assessment-calls/data/repository.server.ts | external:crypto | import | n/a | yes | outward | present |
| E1238 | apps/platform/src/features/assessment-calls/data/repository.server.ts | packages/infrastructure/src/coach-calendar/index.server.ts | import | yes | no | lateral | present |
| E1239 | apps/platform/src/features/assessment-calls/server/assessment-calls-composition.server.ts | packages/infrastructure/src/coach-calendar/index.server.ts | import | yes | yes | inward | present |
| E1220 | apps/platform/src/features/assessment-calls/ui/public/book/booking-confirmation.tsx | apps/platform/src/features/assessment-calls/contracts/call-moment.ts | import | no | no | lateral | present |
| E1223 | apps/platform/src/features/assessment-calls/ui/public/book/call-overview.tsx | apps/platform/src/features/assessment-calls/contracts/call-moment.ts | import | no | no | lateral | present |
| E1221 | apps/platform/src/features/assessment-calls/ui/public/book/call-overview.tsx | packages/config/src/index.ts | import | yes | yes | outward | present |
| E1222 | apps/platform/src/features/assessment-calls/ui/public/book/call-overview.tsx | packages/content/src/index.ts | import | yes | no | lateral | present |
| E1240 | apps/platform/src/features/assessment-calls/ui/public/book/call-overview.tsx | packages/domain/src/assessment-call/index.ts | import | yes | yes | lateral | present |
| E1268 | apps/platform/src/features/assessment-calls/ui/public/book/call-overview.tsx | packages/ui/src/lib/index.ts | import | yes | no | lateral | present |
| E1269 | apps/platform/src/features/assessment-calls/ui/public/book/call-overview.tsx | packages/ui/src/primitives/index.ts | import | yes | no | lateral | present |
| E1225 | apps/platform/src/features/assessment-calls/ui/public/book/slot-picker.tsx | apps/platform/src/features/assessment-calls/contracts/call-moment.ts | import | no | no | lateral | present |
| E1224 | apps/platform/src/features/assessment-calls/ui/public/book/slot-picker.tsx | packages/ui/src/primitives/index.ts | import | yes | no | lateral | present |
...
| E1263 | apps/platform/src/features/store/ui/public/catalog/catalog-view.tsx | packages/ui/src/primitives/index.ts | import | yes | no | lateral | present |
...
| E1226 | apps/platform/src/features/store/ui/public/download/download-page.tsx | packages/ui/src/lib/index.ts | import | yes | no | lateral | present |
...
| E1208 | apps/platform/src/features/waitlist/ui/public/email-form.tsx | packages/ui/src/motion/index.ts | import | yes | no | lateral | present |
| E1227 | apps/platform/src/features/waitlist/ui/public/email-form.tsx | packages/ui/src/primitives/index.ts | import | yes | no | lateral | present |
...
| E1209 | apps/platform/src/root-error-page.tsx | packages/ui/src/layout/index.ts | import | yes | no | lateral | present |
| E1229 | apps/platform/src/root-error-page.tsx | packages/ui/src/primitives/index.ts | import | yes | no | lateral | present |
...
| E1270 | apps/platform/src/surfaces/public-site/pages/pricing.tsx | packages/ui/src/lib/index.ts | import | yes | no | lateral | present |
| E1235 | apps/platform/src/surfaces/public-site/pages/pricing.tsx | packages/ui/src/primitives/index.ts | import | yes | no | lateral | present |
...
| E1230 | apps/platform/src/surfaces/public-site/sections/about/about-content.ts | packages/content/src/index.ts | import | yes | yes | inward | present |
...
| E1210 | apps/platform/src/surfaces/public-site/sections/about/about.tsx | packages/ui/src/primitives/index.ts | import | yes | no | lateral | present |
...
| E1233 | apps/platform/src/surfaces/public-site/sections/footer-cta/footer-cta.tsx | packages/ui/src/primitives/index.ts | import | yes | no | lateral | present |
...
| E1231 | apps/platform/src/surfaces/public-site/sections/hero/hero.tsx | packages/ui/src/lib/index.ts | import | yes | no | lateral | present |
...
| E1232 | apps/platform/src/surfaces/public-site/sections/hero/hero.tsx | packages/ui/src/primitives/index.ts | import | yes | no | lateral | present |
...
| E1271 | apps/platform/src/surfaces/public-site/sections/legal/legal-document-view.tsx | packages/ui/src/lib/index.ts | import | yes | no | lateral | present |
...
| E1272 | apps/platform/src/surfaces/public-site/sections/pricing/bundle-selector.tsx | packages/ui/src/primitives/index.ts | import | yes | no | lateral | present |
...
| E1264 | apps/platform/src/surfaces/public-site/shell/logo.tsx | apps/platform/src/surfaces/public-site/shell/header-appearance.ts | import | no | no | lateral | present |
...
| E1265 | apps/platform/src/surfaces/public-site/shell/public-layout.tsx | apps/platform/src/surfaces/public-site/shell/header-appearance.ts | import | no | no | lateral | present |
...
| E1266 | apps/platform/src/surfaces/public-site/shell/public-navigation.tsx | apps/platform/src/surfaces/public-site/shell/header-appearance.ts | import | no | no | lateral | present |
...
| E1267 | packages/db/src/index.ts | packages/db/src/database-error.ts | import | no | no | lateral | present |
...
| E1241 | packages/domain/src/assessment-call/assessment-call-rules.ts | packages/domain/src/coach-availability/index.ts | import | no | yes | lateral | present |
| E1242 | packages/domain/src/assessment-call/assessment-call.ts | packages/domain/src/assessment-call/assessment-call-rules.ts | import | no | no | lateral | present |
| E1243 | packages/domain/src/assessment-call/book-assessment-call-use-case.ts | packages/domain/src/assessment-call/assessment-call-rules.ts | import | no | yes | inward | present |
| E1244 | packages/domain/src/assessment-call/index.ts | packages/domain/src/assessment-call/assessment-call-rules.ts | import | no | yes | inward | present |
| E1245 | packages/domain/src/assessment-call/list-open-slots-use-case.ts | packages/domain/src/assessment-call/assessment-call-rules.ts | import | no | yes | inward | present |
| E907 | packages/domain/src/cart/index.ts | packages/domain/src/cart/cart.ts | import | no | yes | inward | present |
| E1246 | packages/domain/src/coach-availability/coach-availability.ts | packages/domain/src/coach-availability/time-interval.ts | import | no | no | lateral | present |
| E1247 | packages/domain/src/coach-availability/coach-availability.ts | packages/domain/src/coach-availability/slot-policy.ts | import | no | no | lateral | present |
| E1248 | packages/domain/src/coach-availability/coach-calendar.ts | packages/domain/src/coach-availability/time-interval.ts | import | no | yes | inward | present |
| E1249 | packages/domain/src/coach-availability/index.ts | packages/domain/src/coach-availability/time-interval.ts | import | no | yes | inward | present |
| E1250 | packages/domain/src/coach-availability/index.ts | packages/domain/src/coach-availability/coach-calendar.ts | import | no | yes | inward | present |
| E1251 | packages/domain/src/coach-availability/index.ts | packages/domain/src/coach-availability/slot-policy.ts | import | no | yes | inward | present |
| E1252 | packages/domain/src/coach-availability/slot-policy.ts | packages/domain/src/coach-availability/time-interval.ts | import | no | no | lateral | present |
...
| E1253 | packages/infrastructure/src/coach-calendar/coach-time-reservations.server.ts | packages/db/src/index.ts | import | yes | no | lateral | present |
| E1254 | packages/infrastructure/src/coach-calendar/coach-time-reservations.server.ts | packages/infrastructure/src/coach-calendar/schema.server.ts | import | no | no | lateral | present |
| E1255 | packages/infrastructure/src/coach-calendar/index.server.ts | packages/infrastructure/src/coach-calendar/coach-time-reservations.server.ts | import | no | no | lateral | present |
| E1256 | packages/infrastructure/src/coach-calendar/index.server.ts | packages/infrastructure/src/coach-calendar/postgres-coach-calendar.server.ts | import | no | no | lateral | present |
| E1257 | packages/infrastructure/src/coach-calendar/postgres-coach-calendar.server.ts | packages/db/src/index.ts | import | yes | no | lateral | present |
| E1258 | packages/infrastructure/src/coach-calendar/postgres-coach-calendar.server.ts | packages/domain/src/coach-availability/index.ts | import | yes | no | lateral | present |
| E1259 | packages/infrastructure/src/coach-calendar/postgres-coach-calendar.server.ts | packages/infrastructure/src/coach-calendar/schema.server.ts | import | no | no | lateral | present |
| E1260 | packages/infrastructure/src/coach-calendar/schema.server.ts | packages/db/src/index.ts | import | yes | no | lateral | present |
...
| E1211 | packages/ui/src/layout/dead-end-page.tsx | external:react | import | n/a | no | lateral | present |
| E1212 | packages/ui/src/layout/dead-end-page.tsx | packages/ui/src/primitives/index.ts | import | no | no | lateral | present |
| E686 | packages/ui/src/layout/index.ts | packages/ui/src/layout/app-shell.tsx | import | no | no | lateral | present |
| E1213 | packages/ui/src/layout/index.ts | packages/ui/src/layout/dead-end-page.tsx | import | no | no | lateral | present |
...
| E1217 | packages/ui/src/primitives/alert.tsx | external:react | import | n/a | no | lateral | present |
| E1216 | packages/ui/src/primitives/alert.tsx | packages/ui/src/lib/cn.ts | import | no | no | lateral | present |
...
| E1215 | packages/ui/src/primitives/index.ts | packages/ui/src/primitives/alert.tsx | import | no | no | lateral | present |
...
| E1262 | apps/platform/src/features/assessment-calls/ui/public/book/book-page.tsx | apps/platform/src/features/assessment-calls/ui/public/book/book-page.css | import | no | no | lateral | present |
...
| E1185 | apps/platform/src/features/assessment-calls/ui/public/book/book-page.tsx | apps/platform/src/features/assessment-calls/ui/public/book/call-overview.tsx | import | no | no | lateral | present |
...
| E1186 | apps/platform/src/features/assessment-calls/ui/public/book/book-page.tsx | apps/platform/src/features/assessment-calls/ui/public/book/slot-picker.tsx | import | no | no | lateral | present |
| E1181 | apps/platform/src/features/assessment-calls/ui/public/book/book-page.tsx | apps/platform/src/features/assessment-calls/ui/public/book/step-heading-focus.ts | import | no | no | lateral | present |
...
| E1190 | apps/platform/src/features/assessment-calls/ui/public/book/booking-confirmation.tsx | external:lucide-react | import | n/a | no | lateral | present |
| E1183 | apps/platform/src/features/assessment-calls/ui/public/book/booking-confirmation.tsx | external:react | import | n/a | no | lateral | present |
...
| E1193 | apps/platform/src/features/assessment-calls/ui/public/book/booking-details-form.tsx | external:lucide-react | import | n/a | no | lateral | present |
...
| E1196 | apps/platform/src/features/assessment-calls/ui/public/book/call-overview.tsx | external:lucide-react | import | n/a | no | lateral | present |
...
| E1199 | apps/platform/src/features/assessment-calls/ui/public/book/slot-picker.tsx | apps/platform/src/features/assessment-calls/ui/public/book/slot-calendar.tsx | import | no | no | lateral | present |
| E1200 | apps/platform/src/features/assessment-calls/ui/public/book/slot-picker.tsx | external:react | import | n/a | no | lateral | present |
| E1201 | apps/platform/src/features/assessment-calls/ui/public/book/slot-picker.tsx | packages/ui/src/lib/index.ts | import | yes | no | lateral | present |
| E1182 | apps/platform/src/features/assessment-calls/ui/public/book/step-heading-focus.ts | external:react | import | n/a | no | lateral | present |
...
| E1203 | packages/ui/src/primitives/index.ts | packages/ui/src/primitives/label.tsx | import | no | no | lateral | present |
| E1204 | packages/ui/src/primitives/label.tsx | external:react | import | n/a | no | lateral | present |
| E1205 | packages/ui/src/primitives/label.tsx | packages/ui/src/lib/cn.ts | import | no | no | lateral | present |
