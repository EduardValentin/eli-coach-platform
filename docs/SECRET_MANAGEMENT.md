...
`BOT_DETECTION_PROVIDER=turnstile`, `PRODUCT_EMAIL_PROVIDER=resend` and `PUBLIC_APP_URL=<public https origin>` must be present in the TEST and PROD env files, which `terraform-infra` owns, before the next deploy: `BOT_DETECTION_PROVIDER` is new, the old `PRODUCT_EMAIL_PROVIDER=disabled` value no longer parses, and `PUBLIC_APP_URL` is no longer optional, so a runtime missing any of them fails config validation and the container exits at startup. `ASSESSMENT_CALL_MEETING_LINK=<the coach's real meeting room>` and `ASSESSMENT_CALL_COACH_EMAIL=<the address bookings are sent to>` belong in the same env files, but nothing refuses a runtime that still carries their placeholders: a deploy that forgets them boots and sends every visitor to a room that does not exist.
...
- `ASSESSMENT_CALL_MEETING_LINK` (the coach's own meeting room; the committed default, `https://example.invalid/2f8b41c6a9d7`, names no room, and no runtime refuses it)
...
Every booking resolves to that one meeting room and its join link never
expires, so the room's own lobby is the whole security boundary: the room
**must** have host-admit (knock) enabled, so nobody holding a join link enters
before the coach lets them in, and it must not be attached to a calendar event
with guests, which would let any guest join unadmitted.
