...
Header: date 2026-09-18, commit 7c156e6f, scope apps/platform/src, apps/platform/db, packages/{config,content,db,domain,infrastructure,test-support,ui}/src plus the enforcement layer (tools/dependency-cruiser.config.cjs, tools/dependency-cruiser.tsconfig.json, tools/boundaries.test.mjs, tools/boundary-fixtures/, tools/domain-layout.mjs, tools/domain-layout.test.mjs, tools/domain-layout-fixtures/, knip.json, eslint.config.mjs, workspace package.json export maps, tsconfigs, vite/react-router/vitest configs), mode record update for GEN-191 over `843d8261..7c156e6f` on top of change review run 8 at e8690f45. Rows U1200–U1262 are new in this update; every other row is carried from run 8 and only the rows GEN-191 changed were re-read.
...
| U1200 | assessment-calls/contracts/assessment-calls.ts:module (openSlotsResponseSchema, bookAssessmentCallRequestSchema, bookingSchema, the success/error/response schemas and their inferred types) | C17 | boundary-data | adapters | published (contracts) | visitor | new |
...
| U1206 | assessment-calls/data/repository.server.ts:PostgresAssessmentCallRepository (implements `AssessmentCallReservations`) | C17 | adapter | adapters | public | visitor | new. `reserve` runs `AssessmentCall.decideReservation` inside its own transaction; `findById` classifies Postgres `22P02` as "no such booking" (recorded in `decisions.md`) |
| U1207 | assessment-calls/data/schema.server.ts:assessmentCallsTable | C17 | framework-glue | frameworks | published (drizzle-kit glob) | operator/platform | new. Unique index on `starts_at`, index on `(visitor_email, starts_at)`; migration `db/drizzle/0018_create_assessment_calls.sql` |
| U1208 | assessment-calls/data/static-coach-availability.server.ts:StaticCoachAvailability (implements `CoachAvailabilitySource`) | C17 | adapter | adapters | public | operator/platform | new. Holds one `CoachAvailability.configure(...)` literal and exposes it as `readonly timeZone` as well as through `current()`, which is what keeps the composition synchronous |
...
| U1218 | assessment-calls/email/assessment-call-schedule.server.ts:formatCallMoment | C17 | utility | adapters | public | visitor | new. `Intl`-only; the browser half has its own `formatCallMoment` in U1226 built on `TZDate` |
| U1219 | assessment-calls/email/calendar-invite.server.ts:buildIcs, buildGoogleCalendarUrl | C17 | adapter | adapters | public | visitor, operator/platform | new. Both read `endsAt` off the snapshot rather than re-applying the duration rule |
| U1220 | assessment-calls/server/assessment-calls-composition.server.ts:composeAssessmentCallsFeature (+AssessmentCallsFeature, AssessmentCallsFeatureHandles) | C17 | composition | composition | published (container only) | operator/platform | new. Synchronous: no handle it builds needs awaiting |
...
| U1223 | assessment-calls/ui/public/book/book-page.tsx:AssessmentCallBookingRoute | C17 | view | frameworks | published (routed) | visitor | new |
...
| U1225 | assessment-calls/ui/public/book/booking-flow.ts:module (reduceBookingFlow, INITIAL_BOOKING_FLOW, BookingFlowState, BookingFlowEvent, BookingClientError) | C17 | step machine (pure, tested) | adapters | public | visitor | new |
| U1226 | assessment-calls/ui/public/book/slot-grouping.ts:module (dayKeyOf, groupSlotsByDay, horizonEnd, formatSlotLabel, formatSlotDay, formatCalendarDay, formatCallMoment, describeTimeZone) | C17 | adapter (pure, tested) | adapters | public | visitor | new. The one module that names `@date-fns/tz`; it regroups instants into the display zone with `TZDate` and labels them with `Intl` |
...
| U1235 | assessment-calls/ui/public/join/join-page.tsx:AssessmentCallJoinRoute | C17 | view | frameworks | published (routed) | visitor | new |
...
| U1240 | domain/assessment-call/assessment-call.ts:AssessmentCall (+AssessmentCallProps, AssessmentCallSnapshot carrying the derived `endsAt`, ReservationDecision; `endsAt()`, `isUpcoming(now)`, `isHeldBy(email)`, `toSnapshot()`, statics `reconstitute(props)` / `decideReservation({slotHolder, upcomingCallForEmail})`) | C1 | entity | entities | published | visitor, operator/platform | new. `decideReservation` is the slot-taken / email-has-upcoming ladder the repository calls inside its transaction, the same shape as `Waitlist.decideReducedPricingRegistration` |
| U1241 | domain/assessment-call/assessment-call-reservations.ts:AssessmentCallReservations (+ReserveAssessmentCallCommand, ReservationResult = reserved \| slot_taken \| email_has_upcoming_call) | C1 | port | use-cases | published | operator/platform | new |
...
| U1244 | domain/assessment-call/book-assessment-call-use-case.ts:BookAssessmentCallUseCase (+BookAssessmentCallCommand, BookAssessmentCallResult = booked \| slot_unavailable \| email_already_booked \| closed; takes `bookingOpen`, `clock: Clock`, `logger: Logger`, `coachTimeZone`, uses `EmailAddress`) | C1 | use-case | use-cases | published | visitor, operator/platform | new. `booked` and `email_already_booked` carry `AssessmentCall` instances out to the controller (D9); a notification failure is logged, never surfaced to the visitor. `coachTimeZone` is a construction dependency — see the open question in `decisions.md` |
| U1245 | domain/assessment-call/list-open-slots-use-case.ts:ListOpenSlotsUseCase (+OpenSlotsResult = open \| unavailable \| closed; takes `bookingOpen`, `clock`, `logger`) | C1 | use-case | use-cases | published | visitor | new. The `unavailable` member is the degraded read the booking page renders instead of a 500 (D5, CH-E) |
...
| U1248 | domain/coach-availability/coach-availability.ts:CoachAvailability (+CoachAvailabilityProps, Weekday; `openSlotStarts({now, reservedStarts})`, `isOpenStart({start, now})`, static `configure(props)`) | C1 | entity | entities | published | visitor, operator/platform | new. `configure` is the first throwing factory in the package: it asserts whole-hour windows and known weekdays, and deliberately does not validate `timeZone` — see the open question in `decisions.md` |
| U1249 | domain/coach-availability/coach-availability.ts:ASSESSMENT_CALL_RULES | C1 | policy constant | entities | published | operator/platform | new. The single home of the 30-minute duration, 30-minute buffer, 60-minute step, 30-day horizon and 120-minute lead time (CH-H) |
...
| U1262 | config/concerns/assessment-calls.ts:assessmentCallsShape, AssessmentCallsConfig, refineAssessmentCalls | C3 | boundary-data + refinement | frameworks | published (type through `.`) | operator/platform | new. `ASSESSMENT_CALL_MEETING_LINK` and `ASSESSMENT_CALL_COACH_EMAIL`, both with defaults; the refinement fails a production runtime still carrying the placeholder link |
...
| U1170 | config/runtime-environment.ts:runtimeEnvironmentSchema, RuntimeEnvironment (the nine concern shapes composed, six refinements) | C3 | boundary-data | frameworks | package-internal (the type is published through `.`) | operator/platform | gained the `assessment-calls` shape and `refineAssessmentCalls` |
