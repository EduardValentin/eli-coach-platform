...
Header: date 2026-09-18, commit f4f3dc16, scope apps/platform/src, apps/platform/db, packages/{config,content,db,domain,infrastructure,test-support,ui}/src plus the enforcement layer (tools/dependency-cruiser.config.cjs, tools/dependency-cruiser.tsconfig.json, tools/boundaries.test.mjs, tools/boundary-fixtures/, tools/domain-layout.mjs, tools/domain-layout.test.mjs, tools/domain-layout-fixtures/, knip.json, eslint.config.mjs, workspace package.json export maps, tsconfigs, vite/react-router/vitest configs), mode record update for GEN-191 over `843d8261..7be7d1ad` — the slices recorded at 422c7797, corrected after the review fix round `422c7797..f4f3dc16` — on top of change review run 8 at e8690f45. GEN-191 added rows U1200–U1235, U1240–U1252 and U1260–U1265; U1236–U1239 and U1253–U1259 were never assigned. U1263–U1265 came in with the review fix round (`422c7797..f4f3dc16`), which also retired U1218; every row GEN-191 touched was re-read against f4f3dc16, and the rows the second fix round (`f4f3dc16..7be7d1ad`) touched against 7be7d1ad. Every other row is carried from run 8.
...
| U1200 | assessment-calls/contracts/assessment-calls.ts:module (openSlotsResponseSchema, bookAssessmentCallRequestSchema, bookingSchema, the success/error/response schemas and their inferred types) | C17 | boundary-data | adapters | published (contracts) | visitor | new. The error envelope is a code and a message only: `booking_refused` replaced `email_already_booked` and the `existing` call summary is gone (D-B). A visitor time zone is at most 64 characters, the column's width, is accepted when `Intl.DateTimeFormat` constructs with it, and is stored in the canonical spelling `resolvedOptions().timeZone` returns, since `Intl` accepts case variants and offset spellings; nothing is memoised |
...
| U1203 | assessment-calls/api/assessment-calls-controller.server.ts:AssessmentCallsController (+BookingPageData; loadBookingPage, listSlots, book, resolveJoin) | C17 | adapter | adapters | public | visitor, vendor:cloudflare-turnstile | new. Verifies the Turnstile token before any domain call, serialises the domain `Date`s once into the wire shapes, turns a `closed` use-case status into a 404 and `email_already_booked` into a detail-free 409 `booking_refused` (D-B). `book` wraps `bookAssessmentCall.execute` in a blanket `catch` that logs `assessment_call_booking_failure` through `console.error` and answers 500 `server_error`, the shape of the store acquisition and waitlist controllers |
...
| U1206 | assessment-calls/data/repository.server.ts:PostgresAssessmentCallRepository (implements `AssessmentCallReservations`) | C17 | adapter | adapters | public | visitor | new. `reserve` runs `AssessmentCall.decideReservation` inside its own transaction under a per-email advisory lock, and classifies Postgres `23505` on `ASSESSMENT_CALLS_START_UNIQUE_INDEX` as `{ status: "slot_taken" }` without re-reading the winning row; `findById` classifies Postgres `22P02` as "no such booking". Both classifications are recorded in `decisions.md`; every other error is rethrown |
| U1207 | assessment-calls/data/schema.server.ts:assessmentCallsTable, ASSESSMENT_CALLS_START_UNIQUE_INDEX | C17 | framework-glue | frameworks | published (drizzle-kit glob) | operator/platform | new. Unique index on `starts_at`, declared with the exported name the repository matches a `23505` against, so the index and its classification share one literal; index on `(visitor_email, starts_at)`; migration `db/drizzle/0018_create_assessment_calls.sql` |
...
| U1220 | assessment-calls/server/assessment-calls-composition.server.ts:composeAssessmentCallsFeature (+AssessmentCallsFeature, AssessmentCallsFeatureHandles) | C17 | composition | composition | published (container only) | operator/platform | new. Synchronous: no handle it builds needs awaiting. The config handle is `assessmentCallsConfig` |
...
| U1222 | assessment-calls/ui/public/book/book-page.tsx:loader (+shouldRevalidate, meta) | C17 | adapter | adapters | published (routed) | visitor | new. Reads only `assessmentCallsContext`; a slot-loading failure renders the unavailable state rather than a 500 (D5) |
| U1223 | assessment-calls/ui/public/book/book-page.tsx:AssessmentCallBookingRoute (+the module-private BookingFlow and SlotSelectionStep) | C17 | view | frameworks | published (routed) | visitor | new. `BookingFlow` owns the step machine through `useReducer` and routes between the slot step, the details form and the confirmation; `SlotSelectionStep` groups the open slots by display-zone day and renders the calendar and the day's list |
...
| U1225 | assessment-calls/ui/public/book/booking-flow.ts:module (reduceBookingFlow, INITIAL_BOOKING_FLOW, BookingFlowState, BookingFlowEvent, BookingClientError) | C17 | step machine (pure, tested) | adapters | public | visitor | new. The state carries the selected day and the submitted email as well as the step, the selected slot, the booking and the error |
| U1226 | assessment-calls/ui/public/book/slot-grouping.ts:module (dayKeyOf, groupSlotsByDay, horizonEnd) | C17 | adapter (pure, tested) | adapters | public | visitor | new. The one module that names `@date-fns/tz`; it regroups instants into the display zone with `TZDate`. It words nothing; every wording of a call moment, the slot list's day heading included, is U1263 |
...
| U1229 | assessment-calls/ui/public/book/slot-calendar.tsx:SlotCalendar | C17 | view | frameworks | feature-private | visitor | new. The only consumer of C5 `./calendar` |
| U1230 | assessment-calls/ui/public/book/slot-list.tsx:SlotList | C17 | view | frameworks | feature-private | visitor | new |
| U1231 | assessment-calls/ui/public/book/booking-details-form.tsx:BookingDetailsForm (+BookingDetails) | C17 | mixed | frameworks | feature-private | visitor, vendor:cloudflare-turnstile | new |
| U1232 | assessment-calls/ui/public/book/booking-confirmation.tsx:BookingConfirmation | C17 | view | frameworks | feature-private | visitor | new |
| U1233 | assessment-calls/ui/public/book/unavailable-slots.tsx:UnavailableSlots | C17 | view | frameworks | feature-private | visitor | new |
| U1234 | assessment-calls/ui/public/join/join-page.tsx:loader | C17 | adapter | adapters | published (routed) | visitor | new |
| U1235 | assessment-calls/ui/public/join/join-page.tsx:AssessmentCallJoinRoute | C17 | view | frameworks | published (routed) | visitor | new. Its link home is the C5 `Link` with `placement="standalone"` |
| U1263 | assessment-calls/contracts/call-moment.ts:formatCallMoment, formatCallDay, formatCallTime, formatCallWeekday, describeTimeZone | C17 | utility (pure, tested) | adapters | published (contracts) | visitor, operator/platform | new (87facabb). The single wording of a call's day, time and zone, read by the two email content builders and by four browser modules. `Intl`-only; one `Intl.DateTimeFormat` per part and zone, cached for the life of the process. It sits in `contracts/` because that is the one browser-safe folder both halves may import |
| U1264 | assessment-calls/ui/public/book/call-facts.tsx:CallFacts | C17 | view | frameworks | feature-private | visitor | new (f4f3dc16). States the call length from `ASSESSMENT_CALL_RULES.durationMinutes` rather than a literal |
...
| U827 | packages/ui/src/primitives/link.tsx:Link (+linkVariants) | C5 | view | frameworks | published (`./primitives`) | operator/platform | gained a `placement` axis (`prose`, the default, or `standalone`, a 44-pixel-tall line-level target); `linkVariants` is now a function over the module-private `cva` definition |
...
| U1265 | packages/ui/src/primitives/textarea.tsx:Textarea | C5 | view | frameworks | published (`./primitives`) | visitor | new (f4f3dc16). Reuses `inputClasses` from its sibling `input.tsx`, so a text area and an input share one styling source; its one consumer is the booking details form |
...
| U1260 | packages/ui/src/calendar/calendar.tsx:Calendar (+CalendarProps; the package-private MonthNavButton) | C5 | view | frameworks | published (`./calendar`) | visitor | new. Wraps `react-day-picker`'s `DayPicker` in single-select mode and passes its `timeZone` prop through, which is what makes the vendor convert `today`, `month`, `selected`, `disabled` and `modifiers` into the display zone |
| U1261 | packages/ui/src/calendar/index.ts:module | C5 | framework-glue | frameworks | published (`./calendar`) | operator/platform | new |
| U851 | packages/ui/src/{filters,layout,lib,motion,overlays,primitives}/index.ts:module | C5 | framework-glue | frameworks | published (six of the seven subpath entries; `calendar/index.ts` is U1261) | operator/platform | present |
...
| U1240 | domain/assessment-call/assessment-call.ts:AssessmentCall (+AssessmentCallProps, AssessmentCallSnapshot carrying the derived `endsAt`, ReservationDecision = reserved \| slot_taken \| email_has_upcoming_call; `endsAt()`, `toSnapshot()`, statics `reconstitute(props)` / `decideReservation({slotHolder, upcomingCallForEmail})`) | C1 | entity | entities | published | visitor, operator/platform | new. `decideReservation` is the slot-taken / email-has-upcoming ladder the repository calls inside its transaction, the same shape as `Waitlist.decideReducedPricingRegistration`. A held slot is `slot_taken` whoever holds it, the submitter included (D-C); neither refusal carries the call that caused it. `isUpcoming` and `isHeldBy` were deleted with the same-email path |
| U1241 | domain/assessment-call/assessment-call-reservations.ts:AssessmentCallReservations (+ReserveAssessmentCallCommand, ReservationResult = reserved(call) \| slot_taken \| email_has_upcoming_call) | C1 | port | use-cases | published | operator/platform | new |
...
| U1248 | domain/coach-availability/coach-availability.ts:CoachAvailability (+CoachAvailabilityProps, Weekday; `openSlotStarts({now, reservedStarts})`, `isOpenStart({start, now})`, static `configure(props)`) | C1 | entity | entities | published | visitor, operator/platform | new. `configure` is the first throwing factory in the package: it asserts a time zone `Intl.DateTimeFormat` can construct with, whole-hour windows and known weekdays |
| U1249 | domain/coach-availability/coach-availability.ts:ASSESSMENT_CALL_RULES | C1 | policy constant | entities | published | operator/platform | new. The single home of the 30-minute duration, 30-minute buffer, 30-day horizon and 120-minute lead time (CH-H). The 60-minute step is derived from the duration and buffer constants, so the buffer is enforced as the gap the step leaves after each call; the `bufferMinutes` member itself has no reader |
