...
| U911 | domain/waitlist/waitlist.ts:Waitlist (+WaitlistOffer with a private `"all-bundles"` plan literal, WaitlistAvailability, WaitlistSnapshot, WaitlistConsentVersions, WaitlistSignupPricing; `availability(count)`, statics `availabilityBucketStart(now)` / `decideReducedPricingRegistration(...)`) | C1 | entity | entities | published | operator/platform, visitor | configured waiting-list aggregate; cap and active offer are shared aggregate configuration read directly by U963 and U914, while availability and registration decisions are its rules; the direct readonly offer access and shallow immutability are accepted under R17; system response projection belongs to U963 |
...
| U1167 | config/concerns/waitlist.ts:waitlistShape, WaitlistConfig (`WAITLIST_CAP`, active offer plan and campaign; no mode) | C3 | boundary-data | frameworks | published (type through `.`) | operator/platform | changed (f5d1889c; `WAITLIST_MODE` removed) |
