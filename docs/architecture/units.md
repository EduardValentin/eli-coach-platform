...
| U303 | waitlist/data/repository.server.ts:PostgresWaitlistRepository | C8 | adapter | adapters | public | operator/platform | changed (b3eb2653; one READ COMMITTED transaction per signup under the per-offer advisory lock, lowest free `reduced_slot` in 1..cap, insert inside a savepoint, and a U1211-classified constraint rejection mapped to capacity reached or already registered; no retry loop) |
...
| U1212 | apps/platform/db/drizzle/0019_waitlist_reduced_slots.sql:migration | C15 | framework-glue | frameworks | migration runner | operator/platform | added (b3eb2653; adds `reduced_slot`, backfills reduced entries per offer by `row_number()` over `created_at, id`, then adds the unique index and both checks without `NOT VALID`, so more than N reduced entries fail the migration) |
| U1213 | apps/platform/integration-test-config/migration-test-database.ts:MigrationTestDatabase | tests | test adapter | tests | integration rig | operator/platform | added (b3eb2653; a Postgres container migrated through a named journal entry, for the 0019 backfill test) |
