...
| C2 | PortalPageHeader | [data-parity-root="CoachAssessmentCallsHeader"] header | /coach/assessment-calls → /coach/assessment-calls | default | | | confirmed | Real `features/assessment-calls/ui/coach/assessment-calls/assessment-calls-page.tsx` (plan E6). Root preflight now compares tags: this row must resolve to the `<header>` inside `[data-parity-root="CoachAssessmentCallsHeader"]`, not a wrapper. |
...
| C5 | AssessmentCallsSection | [data-parity-root="AssessmentCallsSection"] | /coach/assessment-calls → /coach/assessment-calls | default all tab, past tab active, empty no calls, empty filters active, search field focus-visible, pager page 2 | | | confirmed | Real `assessment-calls/assessment-calls-section.tsx` (plan E7); covers tabs, search, sort, list, pager, empty state. Prototype-only journey filter is not a row. |
| C6 | AppointmentCard | [data-parity-root="AppointmentCard"] | /coach/assessment-calls → /coach/assessment-calls | upcoming card, today card, past card, card hover | | | confirmed | Real `packages/ui/src/appointments/appointment-card.tsx` (plan Part D). |
...
| C14 | root:[data-parity-root="SelectBundle"] > div.max-w-7xl | [data-parity-root="SelectBundle"] > div.max-w-7xl | /select-bundle?token=… → /select-bundle?token=… | valid link, call-first, cancelled notice | | | confirmed | Prototype `pages/SelectBundle.tsx`; real `features/coaching-sales/ui/public/select-bundle/select-bundle-page.tsx` (plan U5). Both sides carry `data-parity-root="SelectBundle"`, but the prototype puts it on its own `<main>` next to its Navbar while production's page root is a `div` inside the public shell's `<main>`, so the row pairs the content column below it (heading, subtitle, cancelled notice, selector); the call-first banner above it is row C18. |
| C15 | BundleSelector | BundleSelector | /select-bundle?token=… → /select-bundle?token=… | checkout regular, checkout reduced, card selected | | | confirmed | Prototype `components/BundleSelector.tsx`; real `features/coaching-sales/ui/public/bundle-selector/bundle-selector.tsx` (+ `bundle-card.tsx`, `bundle-benefits.tsx`). Public mode renders on /pricing → /pricing (no start choice, no checkout action). Production adds a visually hidden native radio per card plus a covering label so the cards are keyboard-operable; the prototype's cards are click-only. |
| C16 | StartChoice | StartChoice | /select-bundle?token=… → /select-bundle?token=… | empty, error, checked, focus-visible | | | confirmed | Prototype `components/StartChoice.tsx` on `ui/radio-group.tsx`; real `features/coaching-sales/ui/public/select-bundle/start-choice.tsx` on `packages/ui/src/primitives/radio-group.tsx`. Error state is reached by pressing Continue to Checkout with no start chosen. |
| C17 | root:[data-parity-root="CheckoutComplete"] > div | [data-parity-root="CheckoutComplete"] > div | /checkout/complete?session=… → /checkout/complete?order=… | immediate, waiting | | | confirmed | Prototype `pages/CheckoutComplete.tsx` (its root is a `<main>` without the public nav); real `features/coaching-sales/ui/public/checkout-complete/checkout-complete-page.tsx`, a `div` inside the public shell's `<main>`, so the row pairs the confirmation panel. |
| C18 | root:[data-parity="call-first-banner"] | [data-parity="call-first-banner"] | /select-bundle?token=… → /select-bundle?token=… | call-first | | | confirmed | The "A Call Comes First" banner of `pages/SelectBundle.tsx`; real `features/coaching-sales/ui/public/call-first-banner.tsx`, also shown by the confirmation page for an unpaid session (production-only fallback, spec D16). |
