...
Header: date 2026-09-18, commit 7c156e6f, scope apps/platform/src, apps/platform/db, packages/{config,content,db,domain,infrastructure,test-support,ui}/src plus the enforcement layer (tools/dependency-cruiser.config.cjs, tools/dependency-cruiser.tsconfig.json, tools/boundaries.test.mjs, tools/boundary-fixtures/, tools/domain-layout.mjs, tools/domain-layout.test.mjs, tools/domain-layout-fixtures/, knip.json, eslint.config.mjs, workspace package.json export maps, tsconfigs, vite/react-router/vitest configs), mode record update for GEN-191 over `843d8261..7c156e6f` on top of change review run 8 at e8690f45.

## Component graph

Generated from a cold cruise at 7c156e6f (357 modules, 928 dependencies, 0 violations, 0 circular, over 334 in-scope production modules; the per-module edge table is below). Count = distinct importing modules. Component IDs refer to `components.md`. Every count C17 does not appear in is unchanged from run 8 at e8690f45: `features/assessment-calls` is the only component the GEN-191 window added, and nothing else gained or lost a cross-component importer.
...
| C17 assessment-calls | C1 domain | 12 | `/assessment-call`, `/coach-availability`, `/shared` |
...
| C17 assessment-calls | C5 ui | 5 | primitives, lib and the calendar subpath |
...
| infrastructure failure | a business status (capacity, duplicate, availability) | review-owned rule (F88/F89 accepted) | review only, refined to "an adapter never swallows an infrastructure failure into a result-union member; a named expected condition may be classified". Two sanctioned instances at 7c156e6f: ENOENT in `FilesystemProductAssetStore.openConfinedAssetFile`, and Postgres `22P02` in `PostgresAssessmentCallRepository.findById`, where a malformed booking id is a visitor typing a bad URL rather than a database failure. Run 6 read all sixteen catch sites in the adapter ring; GEN-191 added the second classification and two catches that are not adapter catches at all — `ListOpenSlotsUseCase` degrading to `unavailable` and `BookAssessmentCallUseCase` logging a notification failure, both recorded in `decisions.md` |
...
| composition-site | apps/platform/src/features/assessment-calls/server/assessment-calls-composition.server.ts | `StaticCoachAvailability`, `PostgresAssessmentCallRepository`, `ConfiguredMeetingRoomLink`, `createAssessmentCallNotifications(...)`, the three `/assessment-call` use cases and `AssessmentCallsController`. Synchronous: `StaticCoachAvailability` exposes `readonly timeZone`, so `coachTimeZone` is read without awaiting `current()` and the container stays a plain function. `bookingOpen` arrives from the container as `!environment.WAITLIST_MODE` and is held by the two use cases, not by the controller (CH-F) |
...
| The assessment-call rule literals (ASSESSMENT_CALL_RULES: duration, buffer, step, horizon, lead) | C1 `/coach-availability` owns them; `AssessmentCall` reads the duration to derive `endsAt`, `CoachAvailability` reads the rest. C17's email and calendar adapters read `endsAt` off the snapshot instead of re-applying the duration, so the literal has no second home | C1 |
...
An edge from A to B means A's source names B. Direction `inward` points toward policy (ring order: entities, use-cases, adapters, frameworks, composition). Generated from every `import`, `export … from`, dynamic `import()` and CSS `@import` in the 334 in-scope production modules at 7c156e6f; kind is `import` for all rows (the `implements` and `constructs` relationships are recorded in the Boundaries and Entry points sections above). Crosses-ring compares the majority ring of the two modules from `units.md`; an edge into a `packages/domain` folder entry is recorded `lateral`, because a subpath barrel is a publication surface rather than a ring of its own. Externals are tagged framework, vendor or runtime; only the specifiers the cruise resolves appear as `external:` rows, so `react-router`, `motion` and `react-day-picker` are named in the per-component external list above but have no row here. Component membership is by path (see `components.md`). Edge IDs are stable across runs: 491 of these rows keep the ID run 6 gave them, E738-E1013 came in with run 8, and E1014-E1165 came in with GEN-191. A row whose source module GEN-191 only edited keeps its old ID. The generating command is `npx depcruise --config tools/dependency-cruiser.config.cjs --output-type json apps/platform/src packages/{config,content,db,domain,infrastructure,test-support,ui}/src`.
...
| E1042 | apps/platform/src/features/assessment-calls/email/coach-notification-email.server.ts | apps/platform/src/features/assessment-calls/email/assessment-call-schedule.server.ts | import | no | no | lateral | present |
...
| E1062 | apps/platform/src/features/assessment-calls/email/visitor-confirmation-email.server.ts | apps/platform/src/features/assessment-calls/email/assessment-call-schedule.server.ts | import | no | no | lateral | present |
...
| E1096 | apps/platform/src/features/assessment-calls/ui/public/book/book-page.tsx | external:lucide-react | import | n/a | no | lateral | present |
...
| E1100 | apps/platform/src/features/assessment-calls/ui/public/book/booking-confirmation.tsx | apps/platform/src/features/assessment-calls/ui/public/book/slot-grouping.ts | import | no | no | lateral | present |
...
| E1103 | apps/platform/src/features/assessment-calls/ui/public/book/booking-details-form.tsx | apps/platform/src/features/assessment-calls/ui/public/book/slot-grouping.ts | import | no | no | lateral | present |
...
| E1106 | apps/platform/src/features/assessment-calls/ui/public/book/booking-details-form.tsx | external:react | import | n/a | no | lateral | present |
...
| E1110 | apps/platform/src/features/assessment-calls/ui/public/book/booking-details-form.tsx | packages/ui/src/lib/index.ts | import | yes | no | lateral | present |
...
| E1133 | packages/config/src/concerns/assessment-calls.ts | packages/config/src/concerns/app.ts | import | no | no | lateral | present |
