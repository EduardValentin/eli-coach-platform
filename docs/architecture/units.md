...
| U963 | domain/waitlist/get-waitlist-use-case.ts:GetWaitlistUseCase (takes `clock: Clock`, `FeatureFlagReader` and `logger: Logger`; interprets `WAITLIST_MODE`, logs a failed flag read, and assembles `WaitlistSnapshot`) | C1 | use-case | use-cases | published | visitor, operator/platform | changed (63a570a7, 489f6293, 010904d8, f0eb1bf4; owns system projection and names the published `FeatureFlagSet`) |
...
| U1193 | apps/platform/db/drizzle/0018_restore_waitlist_mode_feature_flag.sql:migration | C15 | framework-glue | frameworks | migration runner | operator/platform | added (fc2fd208; seeds `WAITLIST_MODE=true`) |
| U1194 | apps/platform/e2e/support/database.ts:createE2eDatabasePool | tests | test adapter | tests | E2E-private | operator/platform | added (621cdc7e) |
| U1195 | apps/platform/e2e/support/fixtures.ts:test | tests | testing API | tests | E2E journeys | operator/platform, client, coach | changed (621cdc7e) |
| U1196 | apps/platform/e2e/support/global-setup.ts:globalSetup | tests | test composition | tests | Playwright hook | operator/platform | changed (621cdc7e; disables persisted waitlist mode through U1198, which captures the prior value) |
| U1197 | apps/platform/e2e/support/global-teardown.ts:globalTeardown | tests | test composition | tests | Playwright hook | operator/platform | changed (621cdc7e; restores the captured persisted mode in `finally`) |
| U1198 | apps/platform/e2e/support/waitlist-mode.ts:disableWaitlistMode, restoreWaitlistMode | tests | test adapter | tests | E2E-private | operator/platform | added (621cdc7e); changed (f0eb1bf4; setup records the prior `WAITLIST_MODE` value in the runner environment and teardown writes that value back) |
| U1199 | apps/platform/package.json:`files` | C15 | boundary-data | frameworks | package-manager entry | operator/platform | changed (010904d8; production deploy emits only `build`) |
| U1200 | docker/Dockerfile.react-router:post-deploy assertion | enforcement | build enforcement | frameworks | Docker builder | operator/platform | changed (48b8cafb; requires the server build, rejects top-level `src`/`e2e`, recursively rejects workspace test/spec files, and rejects test-support) |
| U1201 | packages/config/package.json:`files` | C3 | boundary-data | frameworks | package-manager entry | operator/platform | changed (48b8cafb; retains `src` except test/spec files) |
| U1202 | packages/content/package.json:`files` | C4 | boundary-data | frameworks | package-manager entry | operator/platform | changed (48b8cafb; retains `src` and `artifacts` except source test/spec files) |
| U1203 | packages/domain/package.json:`files` | C1 | boundary-data | frameworks | package-manager entry | operator/platform | changed (48b8cafb; retains `src` except test/spec files) |
| U1204 | packages/infrastructure/package.json:`files` | C6 | boundary-data | frameworks | package-manager entry | operator/platform | changed (48b8cafb; retains `src` except test/spec files) |
| U1205 | packages/ui/package.json:`files` | C5 | boundary-data | frameworks | package-manager entry | operator/platform | changed (48b8cafb; retains `src` including CSS except test/spec files) |
