...
| U852 | packages/ui/src/layout/navigation-dialog.tsx:NavigationDialog (+NavigationMenu) — owns the mobile navigation pattern: persistent top bar with the trigger/close swap, first-link focus, the closing state, and the desktop-breakpoint close | C5 | view | frameworks | published (`./layout`) | visitor, client, coach | present |
