...
Header: date 2026-09-18, commit 6fc3157f, scope apps/platform/src, apps/platform/db, packages/{config,content,db,domain,infrastructure,test-support,ui}/src plus the enforcement layer (tools/dependency-cruiser.config.cjs, tools/dependency-cruiser.tsconfig.json, tools/boundaries.test.mjs, tools/boundary-fixtures/, tools/domain-layout.mjs, tools/domain-layout.test.mjs, tools/domain-layout-fixtures/, knip.json, eslint.config.mjs, workspace package.json export maps, tsconfigs, vite/react-router/vitest configs), mode record update for GEN-191 over `843d8261..6fc3157f` — the slices recorded at 422c7797, corrected after the two review fix rounds (`422c7797..186b77c4`) and the remediation round (`186b77c4..6fc3157f`) — on top of change review run 8 at e8690f45.

## Component graph

Generated from a cold cruise at 6fc3157f (360 modules, 936 dependencies, 0 violations, 0 circular, 0 unresolved, over 337 in-scope production modules; the per-module edge table is below). Count = distinct importing modules. Component IDs refer to `components.md`. Every count C17 does not appear in is unchanged from run 8 at e8690f45: `features/assessment-calls` is the only component the GEN-191 window added, and nothing else gained or lost a cross-component importer. The review fix round moved two C17 counts: C17 → C1 went 12 → 13 (`ui/public/book/call-facts.tsx` reads the duration off `ASSESSMENT_CALL_RULES`) and C17 → C5 went 5 → 6 (the join page took `Link` from `./primitives`, while `booking-details-form.tsx` dropped its `./lib` import). The remediation round took C17 → C5 back to 5: the join page no longer renders a page of its own, so it imports nothing from C5.
...
| C17 assessment-calls | C5 ui | 5 | `./primitives` (the booking page, the details form, the confirmation and the unavailable state) and `./calendar` (`slot-calendar.tsx`); C17 no longer imports `./lib` |
...
| The call-moment wording (contracts/call-moment.ts: `formatCallMoment`, `formatCallDay`, `formatCallTime`, `formatCallWeekday`, `describeTimeZone(instant, timeZone)`) | C17 only: the two email content builders and four browser modules (`booking-confirmation.tsx`, `booking-details-form.tsx`, `slot-calendar.tsx`, `slot-list.tsx`), so the email and the screen word a call's date, time and zone from one source. `formatCallWeekday` is the year-less day heading of the slot list, the last wording to move into the module. `Intl`-only, with one `Intl.DateTimeFormat` cached per part and zone for the life of the process | C17 |
