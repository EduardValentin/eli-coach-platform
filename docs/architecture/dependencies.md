...
Header: audit 2026-09-15 at commit 148d594f, scope apps/platform, packages, tools, knip.json, eslint.config.mjs; last update 2026-09-23, feature-flag override change review.
...
| C14 server | C8 waitlist | 3 | shared container composition calls `composeWaitlistFeature`; feature contexts and the non-production HTTP adapter publish the waitlist contexts |
| C14 server | C9 accounts | 2 | the container calls `composeAccountsFeature` |
| C14 server | C17 assessment-calls | 3 | shared container composition calls `composeAssessmentCallsFeature`; feature contexts and the non-production HTTP adapter publish the assessment-call contexts |
...
| B192 | FeatureFlagReader (U907) | C1 use-cases | U909 GetFeatureFlagsUseCase in the same module; non-production U536 decorates it | U534 FeatureFlagController; U963 GetWaitlistUseCase; U1300 AssessmentCallBookingWindow; U536 wraps U909 only in enabled LOCAL/TEST artifacts | optional self-contained U544 in; FeatureFlagSet out | U909 in production, U536 on the non-production detail side | dependency-absence, the `./feature-flag` entry and `domain-slices`; the build flag and Docker assertion remove U536/U540/U541 from production artifacts |
...
| B144 | the six feature/runtime request-context keys plus three feature-flag evaluation keys | C7, C8, C9, C17 `server/guards/` and C14 `server/guards/` | `createFeatureContextMiddleware` (U517) sets the six container-backed keys; non-production U540 sets U537-U539 | feature routes and surfaces read only their own published guard keys | a feature slice, `{ appBasePath, botDetection }`, or self-contained U544 plain data | the context key | each is created with `createContext<…>()` and constructs nothing (`guards-construct-nothing`); separate evaluation keys preserve the feature-to-app-server boundary |
...
| B152 | PlatformContainer (U502) composition output | C14 composition | U543, reached by production U500 and non-production U541 | `root.server.ts` through the selected getter | a record of feature slices `{ accounts, assessmentCalls, closeDatabase, platform, store, waitlist }` | root.server.ts | `composition-root`; production artifact assertion removes U541 and its dynamic chunk |
...
|---|---|---|
| composition-root | apps/platform/src/server/container.server.ts (createPlatformContainer, memoised by getPlatformContainer) | the production database, `PostgresFeatureFlagRepository` and raw `GetFeatureFlagsUseCase`, passed to U543; imports no non-production module |
| composition-root (non-production build only) | apps/platform/src/server/non-production/feature-flag-overrides-composition.server.ts | exact LOCAL/TEST runtime gate, database-backed reader decorated by U536, U540 HTTP middleware and U543; absent from the default production artifact |
| composition-root (root) | apps/platform/src/root.server.ts | Clerk, compile-selected lazy override composition or no-op/raw production container, feature contexts and account resolution |
| composition-site | apps/platform/src/server/platform-container-composition.server.ts | shared feature assembly from an explicit database, feature-flag reader and runtime environment; constructs common adapters and all five feature slices without selecting an environment |
...
| `app.feature_flags`, including `WAITLIST_MODE` | C6 declares and reads it; C15 migration 0018 seeds `WAITLIST_MODE=true`; C14 constructs the generic reader and, in LOCAL/TEST only, overlays the request's allowlisted browser-session values after the database read; U1203 and integration tests write it only for test arrangement | C6 owns the table; U963 owns the waitlist-mode interpretation and U1300 the booking one, both through `WAITLIST_MODE_FEATURE_FLAG`; production reads only the stored set |
