...
export function EquivalenceGroups() {
  const { foods, equivalenceGroups, addEquivalenceGroup, assignFoodToGroup } =
    useNutrition();
  ...
  return (
    <div className="flex flex-col gap-6">
      <p className="text-sm text-text-secondary">
        Group interchangeable foods (e.g. "Lean proteins"). A client can swap
        one for another within the same group.
      </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {equivalenceGroups.map((group) => {
          const members = foods.filter(
            (f) => f.equivalenceGroupId === group.id,
          );
          return (
            <Card key={group.id} className="flex flex-col gap-6">
              <CardContent className="pt-6 flex flex-col gap-3">
                <p className={WIDGET_TITLE_CLASS}>{group.name}</p>
                {members.length === 0 ? (
                  <p className="text-sm text-text-secondary">No foods yet.</p>
                ) : (
                    ...
                    {members.map((f) => (
                      ...
                      >
                        <span className="inline-flex items-center gap-2 text-sm text-text-primary">
                          <span
                            className={`size-2.5 rounded-full ${CATEGORY_SWATCH[f.category]}`}
                            aria-hidden="true"
                          />
                          {f.name}
                        </span>
                        <Button
                          variant="ghost"
                          size="xs"
                          onClick={() => assignFoodToGroup(f.id, null)}
                        >
                          Remove
                        </Button>
                      </li>
...
function AddToGroup({ groupId }: { groupId: string }) {
  ...
  return (
    <Select
      value=""
      onValueChange={(foodId) => assignFoodToGroup(foodId, groupId)}
    >
      <SelectTrigger size="sm">
        <SelectValue placeholder="Add a food…" />
      </SelectTrigger>
      <SelectContent>
        {available.map((f) => (
