...
| U908 | domain/feature-flag/feature-flag.ts:FeatureFlag (+package-private FeatureFlagSet, static `toSet(flags)`) | C1 | entity + boundary-data | entities | `FeatureFlag` published through `./feature-flag`; `FeatureFlagSet` package-private | operator/platform | changed (9cce4e16) |
...
| U910 | domain/feature-flag/index.ts:module | C1 | framework-glue | adapters | published (`./feature-flag`; does not publish `FeatureFlagSet`) | operator/platform | changed (9cce4e16) |
...
| U963 | domain/waitlist/get-waitlist-use-case.ts:GetWaitlistUseCase (takes `clock: Clock` and `FeatureFlagReader`; interprets `WAITLIST_MODE` and assembles `WaitlistSnapshot`) | C1 | use-case | use-cases | published | visitor, operator/platform | changed (9cce4e16; owns system projection and derives its private flag-set type from the reader port) |
