...
| C14 server | C3 config | 7 | the container reads `resolveFeatureFlagOverrides`; the override middleware reads `normalizeBasePath` for the cookie path |
...
| E336 | apps/platform/src/server/container.server.ts | packages/config/src/index.ts | import (`RuntimeEnvironment` type; `resolveFeatureFlagOverrides`) | yes | yes | inward | changed (9747d171) |
...
| E1642 | packages/config/src/index.ts | packages/config/src/concerns/feature-flags.ts | import (re-export `resolveFeatureFlagOverrides`) | no | no | lateral | new (9747d171) |
