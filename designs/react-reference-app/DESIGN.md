...
Layout tokens include `container-reading`, `container-content`, and `container-stage`; `size-control-{sm,md,lg}` and `size-avatar-{sm,md,lg}`; `radius-{xs,tile,compact,sm,md,control,panel,phone-frame}`, where `control` is the 14px corner of every button, time slot and calendar control, `compact` the 10px corner of the pricing card's folder-tab badge, and `tile` the 6px corner of small tiles and chat-bubble tails; fully rounded shapes use the framework's `rounded-full`; and `shadow-{soft,card,raised,floating,action,action-hover,phone-frame}`, where `card` is the low elevation of cards and chosen options and `action`/`action-hover` the resting and hover lift of a raised call to action. Public-site compositions use narrowly scoped `public-*` radius, size, and shadow tokens rather than adding raw repeated values.
