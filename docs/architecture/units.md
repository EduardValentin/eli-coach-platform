...
| U1226 | assessment-calls/ui/public/book/slot-grouping.ts:module (dayKeyOf, groupSlotsByDay, horizonEnd, formatSlotDay) | C17 | adapter (pure, tested) | adapters | public | visitor | new. The one module that names `@date-fns/tz`; it regroups instants into the display zone with `TZDate`. Its only label is `formatSlotDay`, the year-less day heading of the slot list; every other wording of a call moment is U1263 |
